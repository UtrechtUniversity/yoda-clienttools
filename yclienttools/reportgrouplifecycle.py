        ...
        for group_object in group_objects:
            ...
            for avu in avus:
                if avu.name in relevant_single_attributes:
                    ...
                    result[avu.name].append(avu.value)
...
def _get_regular_members(session, group_name, attributes):
...
def report_groups_lifecycle(args, session):
    ...
    for group in sorted(_get_research_groups_list(session)):
        ...
        group_managers = ";".join(
            _get_group_managers(
                session, group, attributes))
        regular_members = ";".join(
            _get_regular_members(
                session, group, attributes))
        readonly_members = ";".join(
            _get_readonly_members(
                session, group, attributes))
